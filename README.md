# Cybersecurity Studies

> Repositório pessoal dedicado à documentação da minha jornada em Cibersegurança.  
> Aqui registro anotações, experimentos, referências e aprendizados contínuos.

---

## 📚 Notebooks (NotebookLM)

Utilizo o **NotebookLM** para organizar e aprofundar meus estudos. Acesse os notebooks por tema:

| Tema | Descrição | Link |
|------|-----------|------|
| Geral | Fundamentos e conceitos gerais de cibersegurança | [Acessar notebook](https://notebooklm.google.com/notebook/34d308c3-d7aa-4c77-9820-bea6da87b1d3) |
| OSINT | Técnicas de inteligência em fontes abertas | [Acessar notebook](https://notebooklm.google.com/notebook/fb05a8b6-5ec4-490a-a70c-8f2114e93892) |
| Firewalls | Conceitos, configurações e boas práticas de firewalls | [Acessar notebook](https://notebooklm.google.com/notebook/db48d652-a89f-444d-996a-dd05ba0f678a) |

---

## 📂 Estrutura do Repositório

```
📁 cybersecurity-studies/
├── 📁 geral/           # Fundamentos, conceitos e anotações gerais
├── 📁 osint/           # Ferramentas, técnicas e exercícios de OSINT
├── 📁 firewalls/       # Estudos sobre firewalls e segurança de rede
├── 📁 ctf/             # Write-ups e soluções de CTFs
├── 📁 ferramentas/     # Referências e guias de ferramentas
└── README.md
```

---

## 🗺️ Roadmap de Estudos

- [x] Fundamentos de Redes
- [x] OSINT
- [x] Firewalls
- [ ] Pentest Web
- [ ] Criptografia
- [ ] Análise de Malware
- [ ] CTF

---

## 🔗 Recursos Úteis

- [TryHackMe](https://tryhackme.com)
- [HackTheBox](https://hackthebox.com)
- [OWASP](https://owasp.org)
- [CVE Details](https://cvedetails.com)

---

## ⚠️ Aviso Legal

> Todo o conteúdo deste repositório é estritamente para fins **educacionais**.  
> Não me responsabilizo pelo uso indevido das informações aqui contidas.

---

<p align="center">
  Feito com 🔐 e muita curiosidade.
</p>
